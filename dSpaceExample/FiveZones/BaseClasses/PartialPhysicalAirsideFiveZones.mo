...
partial model PartialPhysicalAirsideFiveZones "Partial model of variable air volume flow system with terminal reheat and five 
...
public
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort THeaCoo(
    ...
    m_flow_nominal=m_flow_nominal,
    allowFlowReversal=allowFlowReversal)
    "temperature sensor between heating coil and cooling coil"
    annotation (Placement(transformation(extent={{140,-50},{160,-30}})));
  Buildings.Fluid.Sensors.VolumeFlowRate VExh(redeclare package Medium =
        MediumA, m_flow_nominal=m_flow_nominal) "Outside air volume flow rate"
    annotation (Placement(transformation(extent={{-70,-62},{-92,-40}})));
equation
  ...
  connect(chicago.weaBus, flo.weaBus) annotation (Line(
      points={{-342.2,180.2},{-326,180.2},{-326,560},{988.714,560},{988.714,506}},
      color={255,204,51},
      thickness=0.5));

  connect(heaCoi.port_b2, THeaCoo.port_a)
    annotation (Line(points={{118,-40},{140,-40}}, color={0,127,255}));
  connect(THeaCoo.port_b, cooCoi.port_a2)
    annotation (Line(points={{160,-40},{190,-40}}, color={0,127,255}));
  connect(eco.port_Exh, VExh.port_a) annotation (Line(points={{-20,-52},{-44,
          -52},{-44,-51},{-70,-51}}, color={0,127,255}));
  connect(VExh.port_b, amb.ports[3]) annotation (Line(points={{-92,-51},{-104,
          -51},{-104,-47.9333},{-114,-47.9333}}, color={0,127,255}));
...
end PartialPhysicalAirsideFiveZones;
