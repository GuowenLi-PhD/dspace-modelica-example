...
model FiveZonesVAV_1i10 "System example for fault injection"
 ...
 extends dSpaceExample.FiveZones.BaseClasses.RunTime;
  ...
  UnitConversion.FromAnalog volToUni_yhea1(
    ...
    y_min=0,
    y_max=1)
    annotation (Placement(transformation(extent={{488,44},{500,56}})));
  Modelica.Blocks.Interfaces.RealInput yHea1 "Voltage signal for heating coil"
    annotation (Placement(transformation(extent={{454,36},{482,64}}),
        iconTransformation(extent={{454,36},{482,64}})));
  ...
  UnitConversion.ToAnolog toV1(
    ...
    x_max=125,
    v_min=0,
    v_max=10) "25/125F mapping to 0-10v"
...
equation
  ...
  connect(yHea1,volToUni_yhea1. v)
    annotation (Line(points={{468,50},{486.8,50}},  color={0,0,127}));
  connect(volToUni_yhea1.y, cor.yVal) annotation (Line(points={{500.6,50},{504,50},
          {504,22},{566,22},{566,34},{566,34}}, color={0,0,127}));
  connect(flo.TRooAir[1], kToF1.K) annotation (Line(points={{1094.14,488.4},{
          1086,488.4},{1086,630},{1098,630}},
                                         color={0,0,127}));
  connect(kToF1.F, toV1.x)
    annotation (Line(points={{1121,630},{1130,630}}, color={0,0,127}));
  connect(toV1.v, TRooAir1_V)
